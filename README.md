...
## Data Model
Our online food delivery data model is made up of 10 unique entities that show our company's processes. Each entity has unique information that allows our company to know who we are delivering to, how we are delivering to them, what we are delivering to them, or even the complaints that our customers have with our services. The ten entities that make up our data model include: Products, OrderProducts, Orders, Restaurants, Reviews, Customers, Memberships, Drivers, SupportTickets, and finally CustomerServiceEmployees.

Products- Products consist of certain details like productID, productName, and menuCategory. The Products entity gives multiple examples of products that people order and what type of food they order. Products have a one-to-many identifying relationship from Products to OrderProducts. Another relationship that exists is a one-to-many non-identifying relationship from Restaurants to Products. Basically restaurants have many products, but these unique products only belong to one, specific restaurant. 
...
We have used the bookmarking storage method. Each query can be called under TP_Qx (x is to be replaced with the query number).
